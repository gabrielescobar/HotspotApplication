 ...
 */
'use strict';
angular.module('zequenzeApp.controllers-custom-login',['ngLoadingSpinner'])
    .controller('CustomLoginController', function (template,$routeParams,localStorageService,login,mac,ip,token_configure,token_get_info,$scope,$rootScope,$location) {
        /*
         Megacable-local
         Megacable-prepaid
         Megacable-Free
        ...
        $scope.goSignUp = function() {
            console.log("entre");
            window.location.replace("#/custom_register");
        };

        $scope.login = function() {
            var timeRemaining = token_get_info.validate(getItem("MAC"));
            timeRemaining.then(function(data) {
                console.log(data);
                if(data=="error"){
                    $scope.tittle = "Error";
                    $scope.message = "Your Device Is Not Registered";
        ...
        section.then(function(data) {
            $scope.templatepage=data[0].customlogin;
        });
